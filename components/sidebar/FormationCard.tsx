...
function FormationOptions({ mode, value, onChange, customModes }: FormationCardProps) {
...
export default function FormationCard({ mode, value, onChange, customModes }: FormationCardProps) {
  const selectedLabel = (() => {
    const cm = customModes.find((m) => m.id === mode);
    if (cm) {
      return cm.formations.find((f) => f.id === value)?.name ?? value;
    }
    return value;
  })();
  ...
  return (
    <AccordionCard label="Formation" selectedLabel={selectedLabel}>
      <FormationOptions mode={mode} value={value} onChange={onChange} customModes={customModes} />
    </AccordionCard>
  );
}
